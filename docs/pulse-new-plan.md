...
## 1. Video pipeline — native AVFoundation + Media3 🔁

> **⛔ `react-native-video-trim` is dropped entirely (2026-06-02).** The current pipeline is **§1.0–§1.0c** below (native AVFoundation / Media3, non-destructive edits). The earlier RNVT plan and the consequence-decisions it forced — `merge`/`trim`/`showEditor`, FFmpegKit setup, the 30 fps cap, the destructive edit model — have been **removed**; only a short historical note remains at the end of this section.
>
