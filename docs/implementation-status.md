...
- [x] **Editor edit-tools behavior (verified, not a bug)** — in RNVT's editor the **crop** icon is a *toggle* drawn at 50% opacity when inactive (full white when active); it is never disabled — tapping it shows the crop overlay. **Undo/redo** start disabled and light up only after a transform (rotate/flip/crop), then cycle correctly. Confirmed live on the sim (crop overlay appears; rotate → undo enables → undo reverts → redo enables). The dim look is intentional styling, left as-is.
- [x] **Orientation bug FIXED via patch-package (2026-06-11)** — per-clip sim testing of all 6 fixtures found **every rotation-tagged portrait clip trimmed sideways**; landscape clips were fine. Root cause in RNVT's re-encode path: it used `-noautorotate` + a manual `transpose` to bake the source rotation into the pixels but **left the source rotation matrix on the output**, so on **ffmpeg-kit 6** players double-rotated it (reproduced exactly on host ffmpeg 6.1.5; `-metadata:s:v:0 rotate=0` does NOT clear the Display-Matrix side-data). Fix: [`patches/react-native-video-trim+8.1.3.patch`](../patches/react-native-video-trim+8.1.3.patch) **removes `-noautorotate` + the source-compensation transpose** and lets FFmpeg **autorotate** (bakes rotation AND strips the matrix). Wired `postinstall: patch-package`. **Re-verified on the sim:** all 4 portrait clips now output upright (`1080×1920` / `2160×3840`, no rotation tag; clip 0 visually upright), both landscape clips stay correct, and a user-rotate composes correctly (portrait→`1920×1080`, no stale tag). Codec→H.264, fps/resolution preserved. _Crop combined with a rotation-tagged source is the one path not separately re-verified; the autorotate change should fix it too since crop now operates on the upright frame._
